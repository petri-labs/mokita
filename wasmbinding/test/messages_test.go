...
func TestCreateDenom(t *testing.T) {
	...
	fundAccount(t, ctx, mokita, actor, actorAmount)
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			// when
			gotErr := wasmbinding.PerformCreateDenom(mokita.TokenFactoryKeeper, mokita.BankKeeper, ctx, actor, spec.createDenom)
			// then
			if spec.expErr {
...
func TestChangeAdmin(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			// Setup
			mokita, ctx := SetupCustomApp(t, tokenCreator)

			// Fund actor with 100 base denom creation fees
			actorAmount := sdk.NewCoins(sdk.NewCoin(types.DefaultParams().DenomCreationFee[0].Denom, types.DefaultParams().DenomCreationFee[0].Amount.MulRaw(100)))
			fundAccount(t, ctx, mokita, tokenCreator, actorAmount)

			err := wasmbinding.PerformCreateDenom(mokita.TokenFactoryKeeper, mokita.BankKeeper, ctx, tokenCreator, &bindings.CreateDenom{
				Subdenom: validDenom,
			})
			require.NoError(t, err)

			err = wasmbinding.ChangeAdmin(mokita.TokenFactoryKeeper, ctx, spec.actor, spec.changeAdmin)
...
func TestMint(t *testing.T) {
	...
	fundAccount(t, ctx, mokita, creator, tokenCreationFeeAmt)

	// Create denoms for valid mint tests
	validDenom := bindings.CreateDenom{
		Subdenom: "MOON",
	}
	err := wasmbinding.PerformCreateDenom(mokita.TokenFactoryKeeper, mokita.BankKeeper, ctx, creator, &validDenom)
	require.NoError(t, err)

	emptyDenom := bindings.CreateDenom{
		Subdenom: "",
	}
	err = wasmbinding.PerformCreateDenom(mokita.TokenFactoryKeeper, mokita.BankKeeper, ctx, creator, &emptyDenom)
	require.NoError(t, err)
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			// when
			gotErr := wasmbinding.PerformMint(mokita.TokenFactoryKeeper, mokita.BankKeeper, ctx, creator, spec.mint)
			// then
			if spec.expErr {
				require.Error(t, gotErr)
				return
...
func TestBurn(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			err := wasmbinding.PerformMint(mokita.TokenFactoryKeeper, mokita.BankKeeper, ctx, creator, mintBinding)
			require.NoError(t, err)
			...
			err = wasmbinding.PerformMint(mokita.TokenFactoryKeeper, mokita.BankKeeper, ctx, creator, emptyDenomMintBinding)
			require.NoError(t, err)

			// when
			gotErr := wasmbinding.PerformBurn(mokita.TokenFactoryKeeper, ctx, creator, spec.burn)
			// then
			if spec.expErr {
				require.Error(t, gotErr)
				return
...
func TestSwap(t *testing.T) {
	...
	poolFunds := []sdk.Coin{
		sdk.NewInt64Coin("umoki", 12_000_000),
	...
	// Estimate swap rate
	umoki := poolFunds[0].Amount.ToDec().MustFloat64()
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			// when
			gotAmount, gotErr := wasmbinding.PerformSwap(mokita.GAMMKeeper, ctx, actor, spec.swap)
			// then
			if spec.expErr {
				require.Error(t, gotErr)
				return
...
func TestSwapMultiHop(t *testing.T) {
	...
	atomPool := preparePool(t, ctx, mokita, actor, poolFunds2)
	...
	umoki := poolFunds[0].Amount.ToDec().MustFloat64()
	ustar := poolFunds[1].Amount.ToDec().MustFloat64()
	expectedOut1 := umoki - umoki*ustar/(ustar+amountIn.Input.ToDec().MustFloat64())

	// Estimate 2nd swap rate
	uatom2 := poolFunds2[0].Amount.ToDec().MustFloat64()
	umoki2 := poolFunds2[1].Amount.ToDec().MustFloat64()
	expectedOut2 := uatom2 - umoki2*uatom2/(umoki2+expectedOut1)
	...
	specs := map[string]struct {
		...
		"self-swap not allowed": {
			swap: &bindings.SwapMsg{
				...
				Route: []bindings.Step{{
					PoolId:   atomPool,
					DenomOut: "umoki", // this is same as the input (output of first swap)
				}},
				Amount: bindings.SwapAmountWithLimit{
					ExactIn: &amountIn,
				},
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			// when
			gotAmount, gotErr := wasmbinding.PerformSwap(mokita.GAMMKeeper, subCtx, actor, spec.swap)
			// then
			if spec.expErr {
				require.Error(t, gotErr)
				return
