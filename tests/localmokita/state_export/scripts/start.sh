...
then
    ...
    echo $MNEMONIC | mokitad init -o --chain-id=$CHAIN_ID --home $MOKISIS_HOME --recover $MONIKER 2> /dev/null
    ...
    VALIDATOR_PUBKEY_JSON=$(mokitad tendermint show-validator --home $MOKISIS_HOME)
    VALIDATOR_PUBKEY=$(echo $VALIDATOR_PUBKEY_JSON | dasel -r json '.key' --plain)
    VALIDATOR_HEX_ADDRESS=$(mokitad debug pubkey $VALIDATOR_PUBKEY_JSON 2>&1 --home $MOKISIS_HOME | grep Address | cut -d " " -f 2)
    VALIDATOR_ACCOUNT_ADDRESS=$(mokitad debug addr $VALIDATOR_HEX_ADDRESS 2>&1  --home $MOKISIS_HOME | grep Acc | cut -d " " -f 3)
    VALIDATOR_OPERATOR_ADDRESS=$(mokitad debug addr $VALIDATOR_HEX_ADDRESS 2>&1  --home $MOKISIS_HOME | grep Val | cut -d " " -f 3)
    VALIDATOR_CONSENSUS_ADDRESS=$(mokitad debug bech32-convert $VALIDATOR_OPERATOR_ADDRESS -p mokivalcons  --home $MOKISIS_HOME 2>&1)
...
mokitad start --home $MOKISIS_HOME --x-crisis-skip-assert-invariants
