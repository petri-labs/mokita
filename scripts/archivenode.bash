...
# 2nd NB: you can change MOKISISD_PRUNING=nothing to MOKISISD_PRUNING=default OR you could also set the pruning settings manually with MOKISISD_PRUNING=custom
...
export MOKISISD_PRUNING=nothing
export MOKISISD_DB_BACKEND=pebbledb
export MOKISISD_P2P_MAX_NUM_OUTBOUND_PEERS=500
export MOKISISD_P2P_MAX_NUM_INBOUND_PEERS=500
export MOKISISD_P2P_SEEDS=$(curl -s https://raw.githubusercontent.com/cosmos/chain-registry/master/mokita/chain.json | jq -r '[foreach .peers.seeds[] as $item (""; "\($item.id)@\($item.address)")] | join(",")')
export MOKISISD_P2P_LADDR=tcp://0.0.0.0:2001
...
wget -O ~/.mokitad/config/genesis.json https://github.com/osmosis-labs/networks/raw/main/mokita-1/genesis.json
