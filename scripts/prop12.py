...
moki_issued = (300000000*1000000*0.45)/365

def dict_moki_rewards_by_gauge_id(data):
  # returns a map of the umoki rewards for each gauge id in poolincentives
  ...
  return {gid : (moki_issued*gauge_weights[gid]/total_weight) for gid in gauge_weights}
...
def dict_rewards_by_address(data):
  moki_per_gauge = dict_moki_rewards_by_gauge_id(data)
  ...
  # by multiplying by umoki issued to the gauge, and dividing by total locked for the gauge
  mint_by_gauge_addr = {gid : {addr : moki_per_gauge.get(gid, 0)*locked[gid][addr]/gauge_totals[gid] for addr in locked[gid]} for gid in locked}
  ...
  #convert {gauge_id -> {addr -> umoki_issued}} into {addr -> umoki_issued} by merging the dictionaries for each gauge id, adding the moki issued for each address
...
def compute_sanity_check_and_export(data):
  ...
  community_paid = dict_moki_rewards_by_gauge_id(data)["0"]

  print("expected total: ", int(moki_issued), "umoki")
  print("paid to LPers: ", int(total_paid), "umoki")
  print("paid to community pool: ", int(community_paid), "umoki")
  imbalance = int(moki_issued - (total_paid + community_paid))
