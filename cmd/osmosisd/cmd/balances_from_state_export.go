...
Example:
	mokitad export-derive-balances ../genesis.json ../snapshot.json
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, unbonding := range stakingGenesis.UnbondingDelegations {
				...
				unbondingMokis := sdk.NewInt(0)
				for _, entry := range unbonding.Entries {
					unbondingMokis = unbondingMokis.Add(entry.Balance)
				}

				acc.UnbondingStake = acc.UnbondingStake.Add(unbondingMokis)
			...
			for _, delegation := range stakingGenesis.Delegations {
				...
				stakedMokis := delegation.Shares.MulInt(val.Tokens).Quo(val.DelegatorShares).RoundInt()

				acc.Staked = acc.Staked.Add(stakedMokis)
...
Example:
	mokitad staked-to-csv ../balances.json ../airdrop.csv
`,
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx := client.GetClientContextFromCmd(cmd)
