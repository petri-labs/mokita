...
// func GetMokiSnapshot(inputFile string) (Snapshot, error) {
...
// 	convertedAddr, err := bech32.ConvertAndEncode("moki", bz)
// 	if err != nil {
// 		panic(err)
// 	}
...
// Both MOKI and ION recipients. If erroring, ensure to 'git lfs pull'

// Example:
// 	mokitad import-genesis-accounts-from-snapshot networks/cosmoshub-3/snapshot.json networks/mokita-1/ions.json output_address.json
// `,
...
// 			// Read snapshot file
// 			snapshot, err := GetMokiSnapshot(args[0])
// 			if err != nil {
// 				return err
// 			}
...
// Example:
// 	mokitad import-genesis-accounts-from-snapshot ../snapshot.json ../ions.json
// 	- Check input genesis:
...
// 			normalizationFactor := genesisParams.AirdropSupply.ToDec().QuoInt(snapshot.TotalMokisAirdropAmount)
...
// 				if !acc.MokiBalanceBase.IsPositive() {
// 					continue
// 				}

// 				// get normalized moki balance for account
// 				normalizedMokiBalance := acc.MokiBalance.ToDec().Mul(normalizationFactor)

// 				// initial liquid amounts
// 				// We consistently round down to the nearest umoki
// 				liquidAmount := normalizedMokiBalance.Mul(sdk.MustNewDecFromStr("0.2")).TruncateInt() // 20% of airdrop amount
...
// 				// claimable balances
// 				claimableAmount := normalizedMokiBalance.Mul(sdk.MustNewDecFromStr("0.8")).TruncateInt()
...
// 			// The total airdrop moki is a smidge short (~1 moki) short of the stated 50M supply.
// 			// This is due to consistently rounding down.
// 			// We place this remaining 1 moki into the community pool at genesis
