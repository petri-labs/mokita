...
var (
	...
	umokiCoin = sdk.NewCoin("umoki", sdk.NewInt(1000000000))
)
...
func (suite *KeeperTestSuite) TestBalancerPoolMultihopSwapExactAmountIn() {
	...
	tests := []struct {
		...
		{
			name: "Swap - [foo -> umoki](pool 1) - [umoki -> baz](pool 2) with a half fee applied, both pools 1 percent fee",
			param: param{
				routes: []types.SwapAmountInRoute{
					{
						PoolId:        1,
				...
				poolAssets:         []sdk.Coins{sdk.NewCoins(fooCoin, umokiCoin), sdk.NewCoins(umokiCoin, bazCoin)},
				poolFee:            []sdk.Dec{poolDefaultSwapFee, poolDefaultSwapFee},
				tokenIn:            sdk.NewCoin("foo", sdk.NewInt(100000)),
				tokenOutMinAmount:  sdk.NewInt(1),
			},
		...
		{
			name: "Swap - [foo -> umoki](pool 1) - [umoki -> baz](pool 2) with a half fee applied, (pool 1) 1 percent fee, (pool 2) 10 percent fee",
			param: param{
				routes: []types.SwapAmountInRoute{
					{
						PoolId:        1,
		...
		{
			name: "Swap - [foo -> umoki](pool 1) - [umoki -> baz](pool 2) - [baz -> bar](pool 3), all pools 1 percent fee",
			param: param{
				routes: []types.SwapAmountInRoute{
					{
						PoolId:        1,
				...
				incentivizedGauges: []uint64{1, 2, 3, 4, 5, 6},
				poolAssets:         []sdk.Coins{sdk.NewCoins(fooCoin, umokiCoin), sdk.NewCoins(umokiCoin, bazCoin), sdk.NewCoins(bazCoin, barCoin)},
		...
		{
			name: "Swap between four asset pools - [foo -> umoki](pool 1) - [umoki -> baz](pool 2), with a half fee applied, both pools 1 percent fee",
			param: param{
				routes: []types.SwapAmountInRoute{
					{
						PoolId:        1,
		...
		{
			name: "Swap between four asset pools - [foo -> umoki](pool 1) - [umoki -> baz](pool 2) - [baz -> bar](pool 3), all pools 1 percent fee",
			param: param{
				routes: []types.SwapAmountInRoute{
					{
						PoolId:        1,
	...
	for _, test := range tests {
		...
		suite.Run(test.name, func() {
			...
			calcOutAmountAsSeparateSwaps := func(mokiFeeReduced bool) sdk.Coin {
				...
				if mokiFeeReduced {
					...
					routeSwapFee, sumOfSwapFees, err := keeper.GetMokiRoutedMultihopTotalSwapFee(suite.Ctx, route)
...
func (suite *KeeperTestSuite) TestBalancerPoolMultihopSwapExactAmountOut() {
	...
	for _, test := range tests {
		...
		suite.Run(test.name, func() {
			...
			// utilizing a cacheContext so the state does not change
			calcInAmountAsSeparateSwaps := func(mokiFeeReduced bool) sdk.Coin {
				cacheCtx, _ := suite.Ctx.CacheContext()
