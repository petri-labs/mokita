...
func GetQueryArithmeticCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Aliases: []string{"twap"},
		Long: mokicli.FormatLongDescDirect(`Query arithmetic twap for pool. Start time must be unix time. End time can be unix time or duration.

Example:
{{.CommandPrefix}} arithmetic 1 umoki 1667088000 24h
{{.CommandPrefix}} arithmetic 1 umoki 1667088000 1667174400
`, types.ModuleName),
...
func GetQueryGeometricCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: mokicli.FormatLongDescDirect(`Query geometric twap for pool. Start time must be unix time. End time can be unix time or duration.

Example:
{{.CommandPrefix}} geometric 1 umoki 1667088000 24h
{{.CommandPrefix}} geometric 1 umoki 1667088000 1667174400
`, types.ModuleName),
		Args: cobra.ExactArgs(4),
		RunE: func(cmd *cobra.Command, args []string) error {
			// boilerplate parse fields
