...
func (server msgServer) LockTokens(goCtx context.Context, msg *types.MsgLockTokens) (*types.MsgLockTokensResponse, error) {
	...
	if lockExists {
		...
		ctx.EventManager().EmitEvents(sdk.Events{
			sdk.NewEvent(
				...
				sdk.NewAttribute(types.AttributePeriodLockID, mokiutils.Uint64ToString(lockID)),
	...
	ctx.EventManager().EmitEvents(sdk.Events{
		sdk.NewEvent(
			...
			sdk.NewAttribute(types.AttributePeriodLockID, mokiutils.Uint64ToString(lock.ID)),
